...
(function () {
  ...
  function drainGqlBuffer() {
    let n = 0;
    for (const t of gqlBuffer) n += extractNamesFromText(t);
    return n;
  }
  ...
  function isProfileHref(href) {
    ...
    if (
      /\/(posts|photos|videos|reel|watch|stories|story\.php|permalink\.php|events|marketplace|gaming|ads|help|settings|privacy|policies|login)/i.test(
        href
      )
    )
      return false;
